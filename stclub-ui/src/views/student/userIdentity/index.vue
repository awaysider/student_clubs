    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['student:userIdentity:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['student:userIdentity:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['student:userIdentity:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['student:userIdentity:export']"
        >导出</el-button>
      </el-col>
...
<!--      <el-table-column label="用户身份主键id" align="center" prop="identityId" />-->
<!--      <el-table-column label="用户id" align="center" prop="userId" />-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="入社日期" prop="joinTime">
...
<script>
import { listUserIdentity, getUserIdentity, delUserIdentity, addUserIdentity, updateUserIdentity } from "@/api/student/userIdentity";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUserIdentity(this.queryParams).then(response => {
        this.userIdentityList = response.rows;
    ...
    handleUpdate(row) {
      this.reset();
      const identityId = row.identityId || this.ids
      getUserIdentity(identityId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户身份管理编号为"' + identityIds + '"的数据项？').then(function() {
        return delUserIdentity(identityIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('student/userIdentity/export', {
        ...this.queryParams
      }, `userIdentity_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
